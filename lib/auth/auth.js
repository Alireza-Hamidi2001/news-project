...
// دریافت کاربر فعلی
// lib/auth/auth.js - اصلاح getCurrentUser
export async function getCurrentUser() {
    ...
    try {
        ...
        console.log(`🔍 [Auth.getCurrentUser] Looking for user ID: ${decoded.id}`);

        // ✅ استفاده از maybeSingle() به جای single()
        const { data: user, error } = await supabaseAdmin
            .from("users")
            .select("*")
            .eq("id", decoded.id)
        ...
        if (!user && decoded.username) {
            console.log(`🔍 [Auth.getCurrentUser] Trying username: ${decoded.username}`);
            ...
            if (userByUsername) {
                console.log(`✅ [Auth.getCurrentUser] Found by username: ${userByUsername.username}`);
                return userByUsername;
            }
        }

        if (error || !user) {
            console.warn(`⚠️ [Auth.getCurrentUser] User not found: ${decoded.id}`);
            return null;
        }

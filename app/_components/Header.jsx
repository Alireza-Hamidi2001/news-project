                    ...
                    <div className="flex gap-4 items-center">
                        ...
                        {user ? (
                            <>
                                ...
                                >
                                    <div className="w-12 h-12 rounded-full bg-linear-to-r from-blue-500 to-purple-600 flex items-center justify-center text-white font-bold text-lg hover:scale-110 transition-transform duration-200 cursor-pointer">
                                        {user.full_name
                                            ?.charAt(0)
                                            .toUpperCase() || "U"}
                                    </div>
                                    <span className="onlineFlasher absolute -bottom-1 right-0 w-4 h-4 rounded-full bg-green-400"></span>
                                </Link>
                                <LogoutButton variant="icon" />
                            </>
                        ) : (
                            <Link href="/login">
                                <LuLogIn className="icons w-9 h-9" />
                            </Link>
                        )}
                    </div>
